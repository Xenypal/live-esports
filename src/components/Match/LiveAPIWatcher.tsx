    ...
    useEffect(() => {
        ...
        if (status.gameIndex === gameIndex) {
            ...
            if (blueDragonMessage) {
                toastArray.push(() => { createToast(true, isPlaying, dragon_blue.default, blueDragonMessage, trueBlueTeam.image) })
                isPlaying = true
            }

            const redDragonMessage = getDragonMessage(status.dragons.red, lastWindowFrame.redTeam.dragons)
            if (redDragonMessage) {
                toastArray.push(() => { createToast(false, isPlaying, dragon_red.default, redDragonMessage, trueRedTeam.image) })
                isPlaying = true
            }
            ...
            blueKillToasts.forEach(killToast => {
                toastArray.push(() => { createToast(true, isPlaying, kill.default, killToast.message, killToast.image, killToast.diff) })
                isPlaying = true
            })
            ...
            redKillToasts.forEach(killToast => {
                toastArray.push(() => { createToast(false, isPlaying, kill.default, killToast.message, killToast.image, killToast.diff) })
                isPlaying = true
            })
        }
    ...
    if (blueTeam) {
        toast.info(
            <div className="toast-watcher">
                <img className="toast-image" src={image} alt="blue team" />
