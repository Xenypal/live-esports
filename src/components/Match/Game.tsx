...
import { advancePlayerDeathTimeline, getPlayerDeathTimerIndicators, PlayerDeathTimeline, PlayerDeathTimerIndicator } from "./deathTimers";
...
export function Game({ firstWindowFrame, lastWindowFrame, lastDetailsFrame, finishedWinner, gameMetadata, gameIndex, eventDetails, outcome, results, items, runes }: Props) {
    ...
    const [playerDeathTimeline, setPlayerDeathTimeline] = useState<PlayerDeathTimeline>({});
    ...
    const previousDeathFrameRef = useRef<WindowFrame | null>(null);
    const playerDeathTimelineKeyRef = useRef("");
    ...
    */
    ...
    useEffect(() => {
        const timelineKey = `${gameIndex}_${firstWindowFrame.rfc460Timestamp}`;
        const isSameTimeline = playerDeathTimelineKeyRef.current === timelineKey;
        setPlayerDeathTimeline(currentTimeline => advancePlayerDeathTimeline(
            firstWindowFrame,
            isSameTimeline ? previousDeathFrameRef.current : null,
            lastWindowFrame,
            isSameTimeline ? currentTimeline : {},
        ));
        playerDeathTimelineKeyRef.current = timelineKey;
        previousDeathFrameRef.current = lastWindowFrame;
    }, [
    ...
    const blueTeamItemValue = getTeamItemValue(lastWindowFrame.blueTeam.participants, resolvedDetailsFrame, safeItems);
    const redTeamItemValue = getTeamItemValue(lastWindowFrame.redTeam.participants, resolvedDetailsFrame, safeItems);
            ...
            <div className="status-live-game-card-content">
                ...
                <div className="live-game-stats-header">
                    ...
                    <div className="live-game-stats-header-status">
                        {HeaderStats(lastWindowFrame.blueTeam, 'blue-team', blueTeamItemValue)}
                        {HeaderStats(lastWindowFrame.redTeam, 'red-team', redTeamItemValue)}
                    </div>
                    ...
                    <table className="status-live-game-card-table">
                        ...
                        <tbody>
                            {lastWindowFrame.blueTeam.participants.map((player: WindowParticipant, index) => {
                                ...
                                const playerItemValue = getPlayerItemValue(championDetails, safeItems);
                                return [(
                                    <tr className="player-stats-row" key={`${gameIndex}_${championsUrlWithPatchVersion}${gameMetadata.blueTeamMetadata.participantMetadata[player.participantId - 1].championId}`}>
                                        ...
                                        <td>
                                            <GoldDisplay
                                                className="player-stats player-gold-total"
                                                itemValue={playerItemValue}
                                                testId={`player-gold-${player.participantId}`}
...
function HeaderStats(teamStats: TeamStats, teamColor: string, itemValue: number) {
    return (
        <div className={teamColor}>
            ...
            <div className="team-stats gold">
                ...
                <GoldDisplay
                    itemValue={itemValue}
                    testId={`${teamColor}-total-gold`}
                    totalGold={teamStats.totalGold}
                />
            </div>
...
function GoldDisplay({
    ...
    return (
        <span className={className} data-testid={testId} title="Total gold (item value)">
            {formatGoldValue(totalGold)} ({formatGoldValue(itemValue)})
        </span>
    );
}

function getTeamItemValue(participants: WindowParticipant[], detailsFrame: DetailsFrame, items: ItemDictionary) {
    return participants.reduce((totalItemValue, participant) => {
        return totalItemValue + getPlayerItemValue(getParticipantDetails(detailsFrame, participant.participantId), items);
    }, 0);
}

function getParticipantDetails(detailsFrame: DetailsFrame, participantId: number) {
    return detailsFrame.participants.find(participant => participant.participantId === participantId);
}

function getPlayerItemValue(championDetails: Participant | undefined, items: ItemDictionary) {
    if (!championDetails) {
        return 0;
    }

    return championDetails.items.reduce((itemValue, itemId) => itemValue + getItemGoldValue(items[itemId]), 0);
}

function getItemGoldValue(item: ItemDictionary[number] | undefined) {
    const itemGold = Number(item?.gold?.total || 0);
    return Number.isFinite(itemGold) ? itemGold : 0;
}

function formatGoldValue(gold: number) {
    return Number(gold).toLocaleString('en-us');
}
