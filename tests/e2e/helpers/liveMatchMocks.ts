...
export function createObservedDeathCountWindowFrames(): WindowFrame[] {
  const gameStartFrame = createObjectiveTimerInitialWindowFrame();
  const targetParticipant = gameStartFrame.redTeam.participants[2];
...
export function createMultiDeathJumpWindowFrames(): WindowFrame[] {
  const gameStartFrame = createObjectiveTimerInitialWindowFrame();
  const targetParticipant = gameStartFrame.redTeam.participants[2];
  ...
  return [
    ...
    withRedParticipant(
      ...
      {
        currentHealth: 0,
        deaths: targetParticipant.deaths + 3,
        level: 18,
        maxHealth: 2650,
      },
    ),
...
export function createFullHealthDeathCountWindowFrames(): WindowFrame[] {
  const gameStartFrame = createObjectiveTimerInitialWindowFrame();
  const targetParticipant = gameStartFrame.redTeam.participants[2];
  ...
  return [
    ...
    withRedParticipant(
      ...
      2,
      {
        currentHealth: targetParticipant.maxHealth,
        deaths: targetParticipant.deaths + 1,
      },
    ),
  ];
...
export function createInitialDeathsWindowFrame(): WindowFrame {
  const gameStartFrame = createObjectiveTimerInitialWindowFrame();
  const targetParticipant = gameStartFrame.redTeam.participants[2];
  ...
  return withRedParticipant(
    ...
    {
      currentHealth: 0,
      deaths: targetParticipant.deaths + 2,
      level: 18,
      maxHealth: 2650,
    },
  );
...
export function createExpiredDeathCountWindowFrames(): WindowFrame[] {
    const gameStartFrame = createObjectiveTimerInitialWindowFrame();
  const firstDeathFrame = withRedParticipant(
    {
      ...gameStartFrame,
      rfc460Timestamp: '2026-04-03T08:00:01.000Z',
    },
    2,
    {
      deaths: 1,
    ...
    {
      ...firstDeathFrame,
      rfc460Timestamp: '2026-04-03T08:00:15.000Z',
    },
  ];
}

export function createRepeatedDeathCountWindowFrames(): WindowFrame[] {
  const gameStartFrame = createObjectiveTimerInitialWindowFrame();
  const firstDeathFrame = createObservedDeathCountWindowFrames()[1];
  ...
  return [
    ...
    withRedParticipant(
      {
        ...firstDeathFrame,
        rfc460Timestamp: '2026-04-03T08:30:30.000Z',
      },
      2,
      {
        deaths: 2,
...
export function createObservedBaronBuffWindowFrames(): WindowFrame[] {
  ...
  return [
    gameStartFrame,
    ...Array.from({ length: 10 }, () => baronTakeFrame),
    baronTakeFrame,
    baronDeathFrame,
    {
      ...baronDeathFrame,
